...
class BXRest {
  ...
  getUsers(ids) {
    ...
    return new Promise((resolve, reject) => {
      ...
      _.callMethod("user.get", { ID: arFind }, true)
        .then((result) => {
  ...
  getCompanies(ids) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const arFind = [];
      ids.forEach((id) => {
        let entity = _.appCache(CacheType.KeyForCompany(id));
        if (entity) {
          arEntities[id] = entity;
        } else {
          arFind.push(id);
      ...
      _.callMethod("crm.company.list", { ID: arFind }, true)
        .then((result) => {
          result.items.forEach((item) => {
            const entity = new Company(item);
            _.appCache(CacheType.KeyForCompany(entity.Id), entity);
            arEntities[entity.Id] = entity;
          });
          resolve(arEntities);
        })
